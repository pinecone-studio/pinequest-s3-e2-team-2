      ...
      <CardContent className="px-5 flex flex-col divide-y divide-[#e8eef4]">
        {slots.map((s, i) => (
          <div key={i} className="py-3 first:pt-0">
            <div className="flex items-center gap-1.5 text-[12px] font-semibold text-[#31A8E0] mb-2">
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-2 mb-0.5">
      <span className="text-[12px] text-[#8a9bb0] min-w-[52px]">{label}</span>
      <span className="text-[12px] text-[#2c3e50] font-semibold">{value}</span>
    </div>
  );
}
