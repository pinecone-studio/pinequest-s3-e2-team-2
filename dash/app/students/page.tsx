    ...
    <TooltipProvider>
      ...
      <div className="bg-white p-4 rounded-2xl border flex flex-wrap items-center gap-3 justify-between">
        <div className="flex items-center gap-3">
          ...
          <Tooltip>
            ...
            <TooltipContent sideOffset={8}>Нарийвчилсан шүүлтүүр нээх</TooltipContent>
          </Tooltip>
        </div>
      </div>
      ...
      {hasAnyFilter && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {scoreFilter.map((s) => (
            <div key={s} className="inline-flex items-center gap-1 rounded-full border border-slate-300 bg-white px-3 py-1 text-sm text-slate-800">
              <span>Оноо: {s}</span>
              <button
                className="text-slate-500 transition-all duration-200 ease-out hover:text-slate-900 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1"
                onClick={() => setScoreFilter((prev) => prev.filter((x) => x !== s))}
              >
                <X className="h-3 w-3" />
              </button>
            </div>
          ...
          <Tooltip>
            ...
            <TooltipContent sideOffset={8}>Идэвхтэй бүх шүүлтийг цэвэрлэнэ</TooltipContent>
          </Tooltip>
        </div>
      )}
      ...
      {filteredItems.length > 0 ? (
        ...
        <div className="rounded-2xl border bg-white p-5">
          <p className="text-sm font-semibold text-slate-900">Илэрц олдсонгүй</p>
          <p className="mt-1 text-sm text-slate-600">
            Одоогоор идэвхтэй шүүлтүүрүүд:
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {courseFilter.map((c) => (
              <span key={`empty-course-${c}`} className="rounded-full border border-slate-300 bg-white px-3 py-1 text-sm text-slate-800">
                {c}
              </span>
            ))}
            {classFilter.map((c) => (
              <span key={`empty-class-${c}`} className="rounded-full border border-slate-300 bg-white px-3 py-1 text-sm text-slate-800">
                {c}
              </span>
            ))}
            {majorFilter.map((m) => (
              <span key={`empty-major-${m}`} className="rounded-full border border-slate-300 bg-white px-3 py-1 text-sm text-slate-800">
                {m}
              </span>
            ))}
            {scoreFilter.map((s) => (
              <span key={`empty-score-${s}`} className="rounded-full border border-slate-300 bg-white px-3 py-1 text-sm text-slate-800">
                Оноо: {s}
              </span>
            ))}
            {!hasAnyFilter && (
